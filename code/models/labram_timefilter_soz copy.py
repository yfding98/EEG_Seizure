...
def mask_topk(x: torch.Tensor, alpha: float = 0.5, largest: bool = True) -> torch.Tensor:
    """Return a keep mask for the top alpha ratio per row. x: [B, H, L, L]."""
    if alpha >= 1.0:
...
class GraphLearner(nn.Module):
    ...
    def forward(self, x: torch.Tensor, masks: torch.Tensor,
        ...
        """x: [B, H, L, D] -> logits, edge_keep_mask, loss."""
        adj = F.gelu(torch.einsum('bhid,bhjd->bhij', self.proj_1(x), self.proj_2(x)))
        knn_mask = mask_topk(adj, alpha, largest=True)
        mask, loss = self.mask_moe(adj, masks, is_training)
        edge_mask = (knn_mask * mask).clamp(max=1.0)
        eye = torch.eye(adj.size(-1), device=adj.device, dtype=edge_mask.dtype)
        edge_mask = torch.maximum(edge_mask, eye.view(1, 1, adj.size(-1), adj.size(-1)))
        return adj, edge_mask, loss
...
class GraphFilter(nn.Module):
    ...
    def forward(self, x: torch.Tensor, masks: torch.Tensor,
        ...
        adj, edge_mask, loss = self.graph_learner(x_h, masks, alpha, is_training)
        if node_valid_mask is not None:
            valid = node_valid_mask.to(device=x.device, dtype=torch.bool)
            edge_mask = edge_mask.bool()
            edge_mask = edge_mask & valid[:, None, :, None] & valid[:, None, None, :]
            eye = torch.eye(L, device=x.device, dtype=torch.bool).view(1, 1, L, L)
            edge_mask = edge_mask | eye
        else:
            edge_mask = edge_mask.bool()

        min_val = torch.finfo(adj.dtype).min
        adj = adj.masked_fill(~edge_mask, min_val)
        adj = torch.softmax(adj, dim=-1)
        if node_valid_mask is not None:
            adj = adj * node_valid_mask.to(device=x.device, dtype=adj.dtype)[:, None, :, None]
...
class GraphBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, masks: torch.Tensor,
        ...
        """x: [B, L, D] → [B, L, D], loss"""
        if node_valid_mask is not None:
            node_mask = node_valid_mask.to(device=x.device, dtype=x.dtype).unsqueeze(-1)
            x = x * node_mask
        out, loss = self.gnn(self.norm1(x), masks, alpha, is_training, node_valid_mask)
        x = x + out
...
class TimeFilterBackbone(nn.Module):
    ...
    def forward(
        ...
        moe_loss = 0.0
        if node_valid_mask is not None:
            node_valid_mask = node_valid_mask.to(device=x.device, dtype=torch.bool)
            x = x * node_valid_mask.to(dtype=x.dtype).unsqueeze(-1)
        for block in self.blocks:
            x, loss = block(x, masks, self._cfg.tf_alpha, is_training, node_valid_mask)
...
class TemporalAttentionPooling(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            patch_valid_mask: optional [B, P] or [B, C, P]
        Returns:
            pooled: [B, C, D]
            attn_weights: [B, C, P]
        """
        ...
        if patch_valid_mask is not None:
            ...
            if valid.dim() == 2:
                valid = valid.unsqueeze(1).expand(-1, x.size(1), -1)
            scores = scores.masked_fill(~valid, torch.finfo(scores.dtype).min)
            weights = F.softmax(scores, dim=-1) * valid.to(dtype=scores.dtype)
...
class SOZLocalizationHead(nn.Module):
    ...
    def forward(
        ...
        if patch_valid_mask is not None:
            valid = patch_valid_mask.to(device=x.device, dtype=torch.bool)
            valid = valid.unsqueeze(1).expand(-1, self.n_channels, -1)

        # 时序注意力池化
        attn_pooled, attn_weights = self.temporal_attn(
            x_4d, valid,
        )   # [B, 22, D], [B, 22, 20]

        # 通道最大池化
        if valid is not None:
            masked_x = x_4d.masked_fill(~valid.unsqueeze(-1), torch.finfo(x_4d.dtype).min)
            max_pooled = masked_x.max(dim=2).values
            has_valid = valid.any(dim=2, keepdim=True)
            max_pooled = torch.where(has_valid, max_pooled, torch.zeros_like(max_pooled))
        else:
