...
# -*- coding: utf-8 -*-
"""
Lightweight_Transformer_BrainNetwork_Integration  (v2)

SOZ detection pipeline — replaces the 12-layer LaBraM pretrained backbone
with a lightweight 2-layer Transformer encoder trained from scratch.

Architecture:
  - SeizureAlignedAdaptivePatching          (Step 1)
  - TemporalConv + 2-layer Transformer
    + TimeFilter                            (Branch A: temporal)
...
try:
    ...
    from .labram_timefilter_soz import (
        TimeFilterBackbone, ModelConfig as LaBraMModelConfig,
    )
...
class IntegrationConfig:
    """Pipeline configuration (v2: lightweight Transformer backbone)."""
    # patching
    n_channels: int = 22
    patch_len: int = 200
    n_pre_patches: int = 5
    ...
    # lightweight Transformer backbone (replaces LaBraM)
    embed_dim: int = 200
    n_transformer_layers: int = 2          # 2-layer lightweight encoder
    ...
    # GAT dropout (shared by TimeFilter and BrainTimeFilter)
...
# =====================================================================
# Patch Embedding (TemporalConv)
# =====================================================================

class TemporalConv(nn.Module):
    """
    3-layer Conv2d patch embedding for EEG signals.

    Input:  [B, N_electrodes, N_patches, patch_size]
    Output: [B, N_electrodes * N_patches, T_out * out_chans]
    """
...
# =====================================================================
# Lightweight 2-layer Transformer Backbone (replaces LaBraM)
# =====================================================================

class LightweightTransformerBackbone(nn.Module):
    """
    2-layer Transformer encoder as a lightweight EEG temporal backbone.

    Replaces the 12-layer LaBraM pretrained model. Trained from scratch
    with the same TemporalConv patch embedding and separate spatial /
    temporal positional encodings.

    Input:  [B, n_channels, n_patches, patch_len]
    Output: [B, n_channels * n_patches, embed_dim]
    """
    ...
    def __init__(
        ...
        t_out = (patch_len + 14 - 15) // 8 + 1
        _temporal_out_dim = t_out * out_chans
        if _temporal_out_dim != embed_dim:
            self.embed_proj = nn.Linear(_temporal_out_dim, embed_dim)
        else:
            self.embed_proj = nn.Identity()

        # ── Positional encoding (spatial + temporal, both learned) ──
        self.pos_embed = nn.Parameter(torch.zeros(1, n_channels, embed_dim))
        self.time_embed = nn.Parameter(torch.zeros(1, n_patches, embed_dim))
...
class Lightweight_Transformer_BrainNetwork_Integration(nn.Module):
    """
    SOZ detection pipeline with a lightweight 2-layer Transformer backbone
    (no LaBraM pretrained weights).
    ...
    def __init__(self, cfg: IntegrationConfig = None):
        ...
        # ── Branch A: Lightweight Transformer + TimeFilter ──
        self.backbone = LightweightTransformerBackbone(
            n_channels=c.n_channels,
            n_patches=max_patches,
            patch_len=c.patch_len,
            embed_dim=c.embed_dim,
            n_heads=c.n_heads_transformer,
            n_layers=c.n_transformer_layers,
            ff_mult=c.ff_mult,
            dropout=c.transformer_dropout,
            out_chans=c.out_chans,
        )
        _tf_cfg = LaBraMModelConfig(
        ...
        self.timefilter = TimeFilterBackbone(_tf_cfg)
    ...
    def freeze_backbone(self):
        """Freeze Transformer backbone, train only heads."""
        for p in self.backbone.parameters():
            p.requires_grad = False
    ...
    def get_param_groups(self, lr: float = 1e-4) -> List[Dict]:
        """Differential LR: backbone 0.5x, timefilter 0.5x, heads 1x."""
    ...
    def summary(self) -> str:
        ...
        return (
            f"Lightweight_Transformer_BrainNetwork_Integration (v2)\n"
            f"  Backbone:         {self.cfg.n_transformer_layers}-layer Transformer\n"
            f"  Embed dim:        {self.cfg.embed_dim}\n"
            f"  Heads:            {self.cfg.n_heads_transformer}\n"
...
def _test():
    ...
    model = Lightweight_Transformer_BrainNetwork_Integration(cfg)
    print(model.summary())
