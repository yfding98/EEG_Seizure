    ...
    def __getitem__(self, idx):
        ...
        if cache_path and cache_path.exists():
            try:
                ...
                if 'valid_patch_mask' in data:
                    ret['valid_patch_mask'] = torch.from_numpy(data['valid_patch_mask']).bool()
...
def collate_fn(batch):
    ...
    if all('brain_nets' in b for b in batch):
        ...
        if all('valid_patch_mask' in b for b in batch):
            ret['valid_patch_mask'] = torch.stack([b['valid_patch_mask'] for b in batch]).bool()
...
def train_one_epoch(
    ...
    for step, batch in enumerate(loader):
        ...
        with torch.amp.autocast('cuda', enabled=scaler is not None):
            ...
            # build aux targets
            vm = outputs.get('valid_patch_mask')
            if vm is None:
...
def evaluate(
    ...
    for batch in loader:
        ...
        if 'valid_patch_mask' in out:
            all_valid_patch_masks.append(out['valid_patch_mask'].cpu().numpy())
    ...
    valid_patch_mask = np.concatenate(all_valid_patch_masks, axis=0) if all_valid_patch_masks else None
